...
test('HEVC requires a positive capability result; unsupported and failed probes use H.264', async () => {
  ...
  assert.equal(
    await preferredVariant(asset, async () => {
      throw Error('probe failed');
    }),
    h264,
  );
  await assert.rejects(
    preferredVariant({ variants: [hevc] }, async () => true),
    /fallback/,
  );
});
test('decoder failure restores time and play state once, without fighting later scrubbing', async () => {
  const video = new Video();
  const controller = createMediaSourceController(video);
  await controller.setSource(undefined, asset, async () => true);
  video.currentTime = 4.5;
  video.paused = false;
  assert.equal(controller.handleError(), true);
  assert.equal(video.src, 'h264.mp4');
  ...
  assert.equal(controller.handleError(), false);
  controller.dispose();
});
test('paused scrub footage remains paused; current user pause overrides a failed playing stream', async () => {
  for (const desired of [undefined, false]) {
    ...
    await controller.setSource(undefined, asset, async () => true);
    video.currentTime = 7;
    video.paused = desired === undefined;
...
test('stale profile probes and disposed probes cannot overwrite the current source', async () => {
  ...
  const controller = createMediaSourceController(video);
  let resolve;
  const pending = controller.setSource(
    undefined,
    asset,
    () =>
      new Promise((r) => {
  ...
  await controller.setSource('mobile.mp4');
  resolve(true);
  await pending;
  assert.equal(video.src, 'mobile.mp4');
  const disposed = controller.setSource(
    undefined,
    asset,
    () =>
      new Promise((r) => {
  ...
  controller.dispose();
  const replacement = createMediaSourceController(video);
  await replacement.setSource('new-mobile.mp4');
  resolve(true);
  await disposed;
  assert.equal(video.src, 'new-mobile.mp4');
  replacement.dispose();
});
test('legacy sources load synchronously and delayed metadata respects the latest playing intent', async () => {
  ...
  const pending = controller.setSource('legacy.mp4');
  assert.equal(video.src, 'legacy.mp4');
  await pending;
  playing = false;
  video.emit('loadeddata');
  assert.equal(video.paused, true);
...
test('an unanswered capability probe falls back within its deadline', async () => {
  const chosen = await preferredVariant(asset, () => new Promise(() => {}), 10);
  assert.equal(chosen, h264);
});
test('empty and pending sources never autoplay or report stale load errors', async () => {
  const video = new Video();
  const controller = createMediaSourceController(video, {
    playing: () => true,
  });
