...
for profile,assets in manifest['profiles'].items():
 for job,asset in assets.items():
  ...
  if job not in ('portal','monitor-idle'):
  ...
  bank=R/'02_render/output/final-table-approved-20w'/profile/job
  def arrays():
   for i in range(count):
    rgb=read_png16(bank/f'{i:05d}.png');q=tracking[profile][i if job=='portal' else 0]['quad'];result=composite(rgb,q)
    if i in (0,120,240):
  ...
  write_rgb_sequences(arrays(),targets,w,h,{'hevc':18,'h264':17},6 if job=='portal' else 30,'p7',30,'nvenc')
  variants=[]
  for codec,target in targets.items():
   info=verify(target,count,30,w,h,codec,6 if job=='portal' else 30)
   info.update(src=f'/media/production/{version}/{profile}/{job}/{target.name}',bytes=target.stat().st_size,sha256=hashlib.sha256(target.read_bytes()).hexdigest(),bitrate=round(target.stat().st_size*8/(count/30)),encoder=codec+'_nvenc',backend='nvenc',quality_mode='cq',quality=18 if codec=='hevc' else 17,gop=6 if job=='portal' else 30,preset='p7',screen_baked=True,hero_sha256=hashlib.sha256((args.hero).read_bytes()).hexdigest())
