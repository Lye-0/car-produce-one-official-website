...
R=Path(__file__).resolve().parents[3];O=R/'output/junction-work-v4';WEB=O/'media'
...
for profile in profiles:
 for job,count,fps in [('drive',600,30),('turn',481,30)]:
  ...
  if job=='turn':
   ...
   manifest=json.loads((R.parent/'01_website/src/media/manifests/journey.json').read_text())
