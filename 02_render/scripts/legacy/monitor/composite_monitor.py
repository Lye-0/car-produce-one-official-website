...
R=Path(__file__).resolve().parents[4];sys.path.insert(0,str(R/'02_render/scripts'))
...
old=json.loads((R/'01_website/src/media/manifests/journey.json').read_text())
...
base=R/'01_website/public'; output=W/'media'
if output.exists():raise FileExistsError('Use a new immutable version name: '+version)
tracking=json.loads((R/'02_render/assets/portal-tracking.json').read_text())
...
for profile,assets in manifest['profiles'].items():
 for job,asset in assets.items():
  ...
  original=old['profiles'][profile].get(job)
  asset['poster']=f'/media/legacy/{version}/{profile}/{job}/00000.webp'
  if job not in ('portal','monitor-idle','monitor-approach'):
   import shutil
   shutil.copy2(base/original['poster'].lstrip('/'),dest/'00000.webp')
   for v in asset['variants']:
    source=base/v['src'].lstrip('/');shutil.copy2(source,dest/source.name)
    v['src']=f'/media/legacy/{version}/{profile}/{job}/{source.name}'
   continue
