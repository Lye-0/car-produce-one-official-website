...
(async () => {
  ...
  try {
    ...
    const hero = await page.locator('main .main-hero').evaluate(node => {
      ...
      clone.querySelector('.portal-hero-background')?.remove();
      clone.classList.remove('portal-live-hero');
      clone.removeAttribute('style');
      const link = clone.querySelector('.portal-hero-link');
      if (link) {
    ...
    const css = fs.readFileSync(path.join(root, '01_website/src/styles/globals.css'), 'utf8');
