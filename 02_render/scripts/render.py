...
counts={'route':2430,'portal':271,'city':1440,**{k:180 for k in holds}}
...
main=bpy.data.scenes['CPO_V18_SITE_MAIN'];city=bpy.data.scenes['CPO_V18_CITY_LOOP']
use_gpu=False
try:
...
for p in profiles:
 ...
 if p=='mobile':w,h=h,w
 for job in (['route'] if action=='test' else jobs):
  s=city if job=='city' else main;bpy.context.window.scene=s;s.render.resolution_x=w;s.render.resolution_y=h
  suffix='City' if job=='city' else 'Idle_'+job.replace('-idle','') if job in holds else 'Route'
  s.camera=bpy.data.objects['V18_'+p+'_'+suffix]
  folder=out/p/job;folder.mkdir(parents=True,exist_ok=True)
  sequence=[(i,f'{i:05d}.png') for i in range(2 if action=='test' else counts[job])]
  ...
  (folder/'complete.json').write_text(json.dumps({'frames':2 if action=='test' else counts[job],'fps':30,'width':w,'height':h,'production':action!='test'}))
