...
OUT=ROOT/'assets/previews/v17';OUT.mkdir(parents=True,exist_ok=True)
...
jobs={
 'camera':(ROOT/'work/v17-motion/camera',241,30,'CPO_v17_camera_2323_30fps.mp4'),
 'loop':(ROOT/'work/v17-motion/loop',384,8,'CPO_v17_city_loop.mp4'),
 'journey':(ROOT/'work/v17-motion/journey',720,8,'CPO_v17_full_journey.mp4'),
}
selected=sys.argv[1:] or list(jobs);results=[]
for name in selected:
    if name=='connection':
        loop=sorted((ROOT/'work/v17-motion/loop').glob('*.png'));arrival=sorted((ROOT/'work/v17-motion/journey').glob('*.png'))[:101]
        ...
        result=encode(loop+loop+bridge+arrival[4:],8,'CPO_v17_two_loops_then_arrival.mp4');result['arrival_cross_dissolve_seconds']=.5;results.append(result);continue
