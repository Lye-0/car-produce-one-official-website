...
out=ROOT/'work/v16-motion'/mode;out.mkdir(parents=True,exist_ok=True)
if mode=='seams':jobs=[(main,0,'main_0000'),(loop,0,'loop_0000'),(loop,1440,'loop_1440')];fps=30;width=640;height=360;samples=48
elif mode=='camera':jobs=[(main,float(f),f'{f-1260:04d}') for f in range(1260,1516)];fps=30;width=768;height=432;samples=16
...
for i,(scene,frame,name) in enumerate(jobs):
 ...
 if i%30==0:print('V16_MOTION_PROGRESS',mode,i,len(jobs),round(time.perf_counter()-start,1),flush=True)
...
print('V16_MOTION_DONE',mode,round(time.perf_counter()-start,1),flush=True)
