...
最新MASTERは20秒・160mの街並みループです。道路に向いた店舗・ホテル・中層建築に、下層部の壁・柱・基礎、大小の広告、背後の建物を追加しました。現在は静止画の確認段階で、長い動画描画は停止しています。走行位置を道路中心から2.6m左へ移し、右折の終点は既存の到着位置に合わせています。確認画像は02_render/reports/streetscape-*.pngです。
