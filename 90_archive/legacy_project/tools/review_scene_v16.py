...
out=ROOT/'work/v16-review';out.mkdir(parents=True,exist_ok=True)
original=s.camera
...
for name,frame,eye,target,lens in shots:
 ...
 s.render.filepath=str(out/(name+'.png'));bpy.ops.render.render(write_still=True);print('V16_REVIEW_SAVED',name,flush=True)
 if eye:bpy.data.objects.remove(c,do_unlink=True)
print('V16_REVIEW_DONE',flush=True)
