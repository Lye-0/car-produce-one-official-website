...
export function firstCoverFrame(
  rows: TrackingFrame[],
  media: Viewport,
  view: Viewport,
) {
  return (
    rows.find(
      (row) =>
        row.visible &&
        rectQuad(view).every((p) =>
          containsPoint(coverQuad(row.quad as unknown as Quad, media, view), p),
        ),
    )?.f ?? 2700
  );
}
export function blendQuad(a: Quad, b: Quad, t: number): Quad {
  return a.map((p, i) => [
    p[0] + (b[i][0] - p[0]) * t,
    p[1] + (b[i][1] - p[1]) * t,
  ]) as unknown as Quad;
...
/** Once the bezel is gone, finish on exact scroll progress rather than quantized frames. */
export function handoffMix(
  presented: number,
  requested: number,
  coverAt: number,
  end = 2700,
) {
  if (presented < coverAt) return 0;
  return Math.min(
    1,
    Math.max(0, (requested - coverAt) / Math.max(1, end - coverAt)),
  );
}
